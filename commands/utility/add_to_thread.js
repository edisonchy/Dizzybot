    ...
    try {
      ...
      if (thread && thread.isThread()) {
        const fetchAllMembers = await interaction.guild.members.fetch();
				const membersWithRole = fetchAllMembers.filter(x => x.roles.cache.has(roleId));
        ...
        membersWithRole.forEach(async (member) => {
          ...
          if (!threadMembers.has(member.id)) {
            try {
              ...
              console.error(`Error adding ${member.user.tag} to the thread:`, error);
            }
          } 
        });
        await interaction.reply('Added all members with the specified role to the thread.');
      }
    } catch (error) {
      console.error('Error fetching the thread:', error);
      await interaction.reply('Error fetching the thread. Please check the thread ID.');
    }
  },
};