...
module.exports = {
    ...
    async function fetchData(interaction) { // add interaction to parameter
      try {
    ...
    function timeTillFriday() {
      ...
      if (currentDay === targetDay && now.getHours() < targetHour) {
        ...
        const hoursUntilFriday = daysUntilFriday * 24 + (targetHour - now.getHours());
